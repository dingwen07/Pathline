...
/** The pure segmentation pipeline behind [TripSegmenter.segment] — see its class doc. */
internal fun segmentSamples(
    samples: List<LocationSampleEntity>,
    classify: (List<LocationSampleEntity>) -> TransportClassification,
): List<SegmentResult> {
    ...
    for (i in usable.indices) {
        ...
        perSample.add(classify(usable.subList(from, to)).mode)
    }
    ...
    return merged.map { range ->
        ...
        val classification = classify(seg)
