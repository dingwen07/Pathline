    ...
    fun write(samples: List<LocationSampleEntity>, out: OutputStream) {
        out.bufferedWriter(Charsets.UTF_8).use { w ->
            ...
            for (s in samples) {
                ...
                s.altitude?.let { w.append("        <ele>").append(it.toString()).append("</ele>\n") }
                w.append("        <time>")
                ...
                if (speed != null || course != null) {
                    w.append("        <extensions>\n")
                    speed?.let { w.append("          <speed>").append(it.toString()).append("</speed>\n") }
                    course?.let { w.append("          <course>").append(it.toString()).append("</course>\n") }
                    w.append("        </extensions>\n")
                }
                w.append("      </trkpt>\n")
            }
