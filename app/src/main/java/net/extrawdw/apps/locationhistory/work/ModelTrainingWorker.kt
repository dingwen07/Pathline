...
class ModelTrainingWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        net.extrawdw.apps.locationhistory.core.AppLog.i(
            "TrainingWorker",
            "training run starting (charging)"
        )
        ...
        val purged =
            trainingRepository.purgeStaleStateExamples() + trainingRepository.purgeStaleTransportExamples()
        if (purged > 0) net.extrawdw.apps.locationhistory.core.AppLog.i(
            "TrainingWorker",
            "purged $purged stale-layout examples"
        )
